% This code is used to store the parts of the simulated data required for determining the temperature model parameters

%%% SILICON %%%
% Use main_PVKpanelT_simpleComparison_AllConditions.m to create dataset for silicon panel
...
for indTamb = 1:length(Tamb)
    ...
    ind = find(TaveCells_vs_SiAll(indTamb,indIrr,indWS,:),1,'last');
    ind1 = find(AbsorptionCoeff_vs_SiAll(indTamb,indIrr,indWS,:),1,'last');

    indT = find(~cellfun('isempty', CutLineBackResults_vs_SiAll(indTamb,indIrr,indWS,:)), 1, 'last'); 
    indx = find(~cellfun('isempty', CutLineBackXResults_vs_SiAll(indTamb,indIrr,indWS,:)), 1, 'last'); 
    
    x=CutLineBackXResults_vs_SiAll{indTamb,indIrr,indWS,indx}*100;
    y=CutLineBackResults_vs_SiAll{indTamb,indIrr,indWS,indT};
    ...
    TaveCellsvsSiAllCond(indDataPoint,4) = TaveCells_vs_SiAll(indTamb,indIrr,indWS,ind)-273.15;
    ...
    AbsCoeffvsSiAllCond(indDataPoint,4) = AbsorptionCoeff_vs_SiAll(indTamb,indIrr,indWS,ind);
...
%%% PEROVSKITE %%%
% Use main_PVKpanelT_simpleComparison_AllConditions.m to create dataset for perovskite panel
...
for indTamb = 1:length(Tamb)
    ...
    ind = find(TaveCells_vs_PSCAll(indTamb,indIrr,indWS,:),1,'last');
    ind1 = find(AbsorptionCoeff_vs_PSCAll(indTamb,indIrr,indWS,:),1,'last');

    indT = find(~cellfun('isempty', CutLineBackResults_vs_PSCAll(indTamb,indIrr,indWS,:)), 1, 'last'); 
    indx = find(~cellfun('isempty', CutLineBackXResults_vs_PSCAll(indTamb,indIrr,indWS,:)), 1, 'last'); 
    
    x=CutLineBackXResults_vs_PSCAll{indTamb,indIrr,indWS,indx}*100;
    y=CutLineBackResults_vs_PSCAll{indTamb,indIrr,indWS,indT};
    ...
    TaveCellsvsPSCAllCond(indDataPoint,4) = TaveCells_vs_PSCAll(indTamb,indIrr,indWS,ind)-273.15;
    ...
    AbsCoeffvsPSCAllCond(indDataPoint,4) = AbsorptionCoeff_vs_PSCAll(indTamb,indIrr,indWS,ind);
